...
def goBack():
    ...
    if active_page == 4 and not thread_event.is_set():
        ...
        if webcam is not None:
            webcam.release()
            webcam = None  # Set webcam to None after releasing
...
def basicPageSetup(pageNo):
    ...
    heading = tk.Label(pages[pageNo], fg="white", bg="#202d42", font="Arial 20 bold", pady=15)
    heading.pack()
...
def register(entries, required, menu_var):
    ...
    # Setting Directory
    path = os.path.join('C:\\Users\\SHUBHAM MISHRA\\Documents\\Shubham Mishra Python Projects\\Criminal-Identification-System-master\\face_Samples', 'temp_criminal')
    ...
    if(len(no_face) > 0):
        ...
        if(rowId > 0):
            ...
            shutil.move(path, os.path.join('C:\\Users\\SHUBHAM MISHRA\\Documents\\Shubham Mishra Python Projects\\Criminal-Identification-System-master\\face_Samples', entry_data["Name"]))
...
def getPage1():
    ...
    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#2196f3",
           ...
           activeforeground="white").grid(row=0, column=0, padx=15, pady=15)
...
def showCriminalProfile(name):
    ...
    path = os.path.join('C:/Users/SHUBHAM MISHRA/Documents/Shubham Mishra Python Projects/Criminal-Identification-System-master/profile_pics', "criminal %d.png"%id)
...
def selectImage():
    ...
    if(len(path) > 0):
        ...
        img_size =  left_frame.winfo_height() -20
...
def webcam(model, names,img_size=(600,600)):
    # Specify the backend for the VideoCapture
    ...
    try:
        while True:
            ...
            showImage(frame, (left_frame.winfo_width(), left_frame.winfo_height()))
        ...
        try:
            left_frame.winfo_exists() and left_frame.destroy()
        except tk.TclError:
            pass
...
def getPage4(path):
    p = path
    # print(p)
    global active_page, video_file, left_frame, right_frame, thread_event, heading
    active_page = 3
    pages[3].lift()
    ...
    thread = threading.Thread(target=videofile, args=(p, model, names))
    thread.start()
...
def getPage3():
    ...
    tk.Button(btn_grid, text="Upload Video", command=selectvideo, font="Arial 15 bold", padx=20, bg="#2196f3",
            fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#091428",
            activeforeground="white").grid(row=0, column=0, padx=25, pady=25)
...
def getPage5():
    ...
    right_frame.configure(text="Detected Criminals")
    left_frame.configure(pady=50)

    (model, names) = train_model()
    print('Training Successful. Detecting Faces')

    thread_event = threading.Event()
    thread = threading.Thread(target=webcam, args=(model, names))
    thread.start()
    
######################################## Home Page ####################################
tk.Label(pages[0], text="Criminal Face Identification System", fg="white", bg="#202d42",
...
tk.Button(btn_frame, text="Register Criminal",command=getPage1).pack(side="left", padx=20)
tk.Button(btn_frame, text="Detect Criminal",command=getPage2).pack(side="left", padx=20)
tk.Button(btn_frame, text="Live Surveillance",command=getPage5).pack(side="left", padx=20)
tk.Button(btn_frame, text="Video Surveillance",command=getPage3).pack(side="left", padx=20)

for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#2196f3", fg="white",
    pady=12, bd=0, highlightthickness=0, activebackground="#091428", activeforeground="white")
    btn.pack(pady=30)
